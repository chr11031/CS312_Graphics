    ...
    while(SDL_PollEvent(&e)) 
    {
		...
		if(e.type == SDL_MOUSEMOTION)
		{
			...
			if(cur == SDL_DISABLE)
			{
				...
				myCam.yaw += mouseX * 0.07;
				myCam.pitch += mouseY * 0.07;
			}
		}
		...
		{
			myCam.z += (cos((myCam.yaw / 180.0) * M_PI)); // * 0.5 to slow it down if you'd like
			myCam.x += (sin((myCam.yaw / 180.0) * M_PI));
		}
		if((e.key.keysym.sym == 's' && e.type == SDL_KEYDOWN))
		{
			myCam.z -= (cos((myCam.yaw / 180.0) * M_PI));
			myCam.x -= (sin((myCam.yaw / 180.0) * M_PI));
		}
		if((e.key.keysym.sym == 'a' && e.type == SDL_KEYDOWN))
		{
			myCam.x -= (cos((myCam.yaw / 180.0) * M_PI));
			myCam.z += (sin((myCam.yaw / 180.0) * M_PI));
		}
...
void DrawTriangle(Buffer2D<PIXEL> & target, Vertex* const triangle, Attributes* const attrs, Attributes* const uniforms, FragmentShader* const frag)
{
    ...
    // Compute first, second, third X-Y-Z pairs: X and Y are used for the pixel, and those plus Z are used in finding N for PHONG
    double firstVec[] = {triangle[1].x - triangle[0].x, triangle[1].y - triangle[0].y, triangle[1].z - triangle[0].z};
    double secndVec[] = {triangle[2].x - triangle[1].x, triangle[2].y - triangle[1].y, triangle[2].z - triangle[1].z};
    double thirdVec[] = {triangle[0].x - triangle[2].x, triangle[0].y - triangle[2].y, triangle[0].z - triangle[2].z};

    double normalized[][3] = { {firstVec[0], firstVec[1], firstVec[2]},
                               {secndVec[0], secndVec[1], secndVec[2]},
                               {thirdVec[0], thirdVec[1], thirdVec[2]} };

    normalize(normalized);

    // inserting N for each vertex for PHONG
    findN(normalized[0], normalized[1], attrs[0]);
    findN(normalized[1], normalized[2], attrs[1]);
    findN(normalized[2], normalized[0], attrs[2]);
